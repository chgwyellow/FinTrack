...
你可以管理現金與其他資產、負債、台美股投資、股利、定期投資計畫、外幣餘額，以及收入與支出項目，並讓財務資料庫保留在自己的 Mac 上。
...
將資產、負債、投資與淨值集中查看，不必分散管理多份清單或試算表。可直接編輯餘額，也可用增加／減少調整記錄餘額變化，並依資產或負債類型分組。建立快照後，可以觀察淨值如何隨時間變化。

### 追蹤投資組合

追蹤台股、美股與 ETF，並以原始幣別記錄持股及實際買入交易。查看投資組合市值、資產配置、平均成本、資本損益與每日損益。股利會獨立記錄並納入投資總報酬。
...
記錄與編輯持股股利，並在損益表整理收入、支出與儲蓄。每個區段最多支援
大項目及其下一層明細。最底層項目可選擇連結既有帳戶，或使用並保存自訂
帳戶名稱。

### 管理外幣

管理外幣餘額與交易（包含換匯），查看目前匯率、平均匯率及各幣別換算成
新台幣的價值。

### 建立自己的淨值歷史

建立手動或定時淨值快照，並以 1 個月、3 個月、6 個月、1 年或全部歷史區間查看資料；可在圖表內左右捲動查看較早的紀錄。Y 軸依所選時間區間的數值調整。手動快照記錄當天日期；定時快照記錄前一個日曆日。同一天再次建立快照會覆蓋該日資料。
...
開啟相關頁面時，程式會更新股價與匯率。定時淨值快照依「設定」中的時間
執行；Mac 必須可用且使用者工作階段已登入。若錯過排程，FinTrack 再次執行
時可以補回。定時快照日期為前一個日曆日；按下「快照」則記錄當天。

## 從原始碼建置

需求：macOS 14 或更新版本，以及 Swift 6（Xcode Command Line Tools）。

```bash
swift build -c release
```

要打包 Apple Silicon app 與 ZIP（預設為 ad-hoc 簽章，並非 Apple Developer ID
簽章）：
...
產物會放在 `releases/`。簽章與發布流程請參閱
[`docs/releasing.md`](docs/releasing.md)。

## 當前限制

- 僅支援 macOS；目前發布檔僅支援 Apple Silicon
- 目前版本尚未使用 Apple Developer ID 簽章、尚未公證，也沒有自動更新功能
